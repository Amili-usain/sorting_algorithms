...
This project aims at learning
- atleast four different sorting algorithms,
- what Big O notation is, how to evaluate the time complexity of a algorithm,
- How to select the best sorting algorithm for a given input and
- What stable sorting algorithm is.
...
* **10. Bitonic sort**
  ...
  * Assumes that `size` is a power of 2 (ie. `size` can be expressed as `2^k`
  where `k >= 0`).
  * use of `printf` was allowed for the task.
  * Prints subarrays each time they are merged.
...
* **12. Dealer**
  ...
  * The  C standard library function `qsort` is not allowed for this task.
